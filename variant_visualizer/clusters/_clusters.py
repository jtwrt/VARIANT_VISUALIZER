...
class ClusterGenerator():
    ...
    config.yml
    """
    def __init__(self, process_number: int, n_processes: int ):
        ...
        gtf_cluster_ids = []
        k = 0
        for i, cluster_id in enumerate(all_gtf_cluster_ids):
            if i == k*n_processes + process_number:
                gtf_cluster_ids.append(cluster_id)
                k += 1

        #print('Generating GTF slices ...')
        step_cluster_ids = [i for i in gtf_cluster_ids if 
        ...
        for cluster_id in gtf_cluster_ids:
